...
Map.addMadamNazar = function ()
{
    if(enabledTypes.includes('nazar'))
    {
        ...
        marker.bindPopup(`<h1>${languageData[lang]['madam_nazar.name']} - ${nazarCurrentDate}</h1><p>Неактуально? Найдите её тут: <a href='https://twitter.com/MadamNazarIO' target="_blank">@MadamNazarIO</a>.</p>`);
        markersLayer.addLayer(marker);
    }
};
